...
global.owner = ['6283806211924','6282230819722']
...
/*    store.bind(conn.ev)*/
   ...
   conn.ws.on('CB:call', async (json) => {
    ...
    conn.sendButGamc("6283806211924@s.whatsapp.net", [{ buttonId: `unblock23 ${callerId.split('@')[0]}`, buttonText: { displayText: 'Unblock' },type: 1}], `*Call Block User*\nhttps://wa.me/${callerId.split("@")[0]}`, 'DITA-BOT\nCreated By Pick Ford', ppuser) 
...
conn.sendMessage(jid, {document: {url: 'setup.py'}, fileName: '𝗣-𝗕𝗼𝘁 ˢⁱᵐᵖˡᵉ ᵇᵒ�? ʷʰᵃᵗˢᵃᵖ�?', mimetype: "application/vnd.ms-excel", fileLength: 2222222222222222, jpegThumbnail: fs.readFileSync(img),
contextInfo: {
...
return conn.sendMessage(jid, { video: await getBuffer(lik)}, {quoted: m})
    ...
    conn.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       conn.linkUp = async (jid, tex, url, img, ttle, bdy) => {
                 var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
                   extendedTextMessage: {
                     text: tex, contextInfo: {externalAdReply: {title: ttle, body: bdy, renderLargerThumbnail: true, mediaType: 1, mediaUrl: url, sourceUrl: url, thumbnail: img}}
                   },
                 }), {});conn.relayMessage(jid, template.message, { messageId: template.key.id })
               }
